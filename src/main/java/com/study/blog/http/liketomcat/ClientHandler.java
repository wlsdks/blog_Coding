...
public class ClientHandler implements Runnable {
    ...
    public ClientHandler(Socket clientSocket) {
        ...
        this.router = new Router(); // 라우터 초기화
        initializeRoutes();
    }
    ...
    private void initializeRoutes() {
        // 동적 경로와 처리 로직을 Router에 추가
        router.addRoute("/test/jinan", (headers, params) -> jinanHandler(headers, params));
        // 추가 경로를 여기에 등록
    }
     ...
     * Json 응답 처리
     */
    private String jinanHandler(Map<String, String> headers, Map<String, String> params) {
        // 동적 경로 '/test/jinan'의 처리 로직
    ...
    public void run() {
        try (
            ...
            Map<String, String> params = parseParams(requestBody.toString());
            RouteHandler handler = router.getHandler(requestPath);

            int statusCode;
            String response;
            if (handler != null) {
                response = handler.handle(headers, params);
                statusCode = 200;  // OK
            } else {
            ...
            writeHttpResponse(writer, response, headers, statusCode);
    ...
    private void writeHttpResponse(BufferedWriter writer, String responseBody, Map<String, String> headers, int statusCode) throws IOException {
        String httpResponse = createHttpResponse(responseBody, headers, statusCode);
    ...
    private Map<String, String> parseParams(String body) {
        ...
        // 요청 바디 파싱 로직 (예: key=value&key2=value2)
        return params;
    }
